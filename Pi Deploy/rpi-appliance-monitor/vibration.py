...
def send_alert(message):
    if len(message) > 1:
        ...
        if len(pushover_user_key) > 0 and len(pushover_app_key) > 0:
            ...
            # pushover(pushover_user_key, pushover_app_key, message, 'DawnsPhone', pushover_sound)
...
def heartbeat():
    ...
    global wififlag
    global wifiLight
    global wifiLoopCounter

    current_time = time.time()
    logging.debug("HB at {} loop {} osflag {} wifiLoopCounter {} wififlag {} wifiLight {} ".format(current_time, loopcounter, osflag, wifiLoopCounter, wififlag, wifiLight ))

    wifiLoopCounter = wifiLoopCounter + 1
    if wifiLoopCounter == 5:
        # Test iNet connect
        try:
